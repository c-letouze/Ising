...
def estimate_correlation_time(nb_init, shape, param):
    """
    This is a DocString"""
    print("Initialization: 'random'")
    t_random = 0.
    for i in range(nb_init):
        ising_random = init_simulation(shape, init='random')
        t_random += relax(ising_random, param)
    t_random /= nb_init
    print("Average time: {:.2f}".format(t_random))
    # initialize the system in 'uniform' configuration
    print("Initialization: 'uniform'")
    t_uniform= 0.
    for i in range(nb_init):
        ising_uniform = init_simulation(shape, init='uniform')
        t_uniform += relax(ising_uniform, param)
    t_uniform /= nb_init
    print("Average time: {:.2f}".format(t_uniform))
    # ~ # keep the longest correlation time
    correlation_time = max(t_random, t_uniform)
    print("Estimated correlation time: {:.1f}".format(correlation_time))
...
mag_moment = 1.0
# convenient tuple
param = (beta, coupling, mag_moment)
# geometry
width = 40
shape = (width, width)

obj = init_simulation(shape, 'random')
hist = monitor(obj, 500, param)
plt.plot(hist[:, 1])
plt.show()
...
# ~ # generate samples
# ~ nb_samples = 10
# ~ path = './IsingData/L{}_T{}.pkl'.format(shape, temp)
# ~ t0 = time.time()
# ~ generate_samples(shape, t_corr, nb_samples, param, path)
# ~ t1 = time.time()
# ~ print("run in {:.2f}s".format(t1-t0))
